...
Stack* create_stack(int capacity) {
	...
	stack->data = (float*)my_malloc(capacity * sizeof(float));
	stack->top = -1;
...
void push(Stack* stack, float value) {
	if (stack->top >= stack->capacity - 1) {
		...
		float* new_data = (float*)my_malloc(new_capacity * sizeof(float));
...
void quick_sort_iterative(float arr[], int low, int high) {
	if (low >= high) return;
...
float* read_data(const char* filename, int* count) {
...
void write_data(const char* filename, float arr[], int count) {
	remove(filename);
...
int verify_sorted(float arr[], int count) {
	for (int i = 1; i < count; i++) {
		if(arr[i] < arr[i - 1]) {
			printf("wrong arr[%d] = %f > arr[%d] = %f", i-1, arr[i-1], i, arr[i]);
			return 0;
}
}
	return 1;
